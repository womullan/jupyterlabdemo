...
c.JupyterHub.base_url = '/jld/'
#c.NCSALDAPAuth.server_address = 'ldap.ncsa.illinois.edu'
#c.NCSALDAPAuth.bind_dn_template = 'uid={username}, ou=People, dc=ncsa, dc=illinois,dc=edu'
#c.NCSALDAPAuth.allowed_groups = [
...
class LSSTSpawner(kubespawner.KubeSpawner):
    ...
    def get_pod_manifest(self):
        ...
        if s_idx != -1:
            ...
            c_idx = image_name.find(':')
        if c_idx > 0:
            tag = image_name[(c_idx + 1):]
            image_name = image_name[:c_idx]
